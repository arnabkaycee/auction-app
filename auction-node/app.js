...
app.use(function (req, res, next) {
	...
	if (req.originalUrl.indexOf('/users') >= 0 || req.originalUrl.indexOf('/tokens') >= 0) {
		return next();
	}
...
app.post('/users', async function (req, res) {
	...
	let response = await helper.getRegisteredUser(username, orgName, null, true);
...
app.get('/tokens', async function (req, res) {
	...
	let tokenJson = require(filePath);
	let response = [];
	for (let key in tokenJson) {
		if (tokenJson[key]["user"]["userId"] == "auctionhouse") continue;
		response.push(tokenJson[key]);
	}
	res.json(response);
});
...
app.get('/user', async function (req, res) {
	...
	let message = await query.queryChaincode(hfc.getConfigSetting('peers')[0], hfc.getConfigSetting('channelName'), hfc.getConfigSetting('chaincodeName'), "getUser", args, req.username, req.orgname);
	res.json(JSON.parse(message));
});
...
app.get('/assets', async function (req, res) {
	...
	let message = await query.queryChaincode(hfc.getConfigSetting('peers')[0], hfc.getConfigSetting('channelName'), hfc.getConfigSetting('chaincodeName'), "getAssetsForUser", args, req.username, req.orgname);
	res.json(JSON.parse(message));
});

// Invoke transaction on chaincode on target peers
app.post('/place-bid', async function (req, res) {
	var args = req.body;
	if (!args) {
		res.send("error");
	}
	args = JSON.stringify(args);
	let message = await invoke.invokeChaincode(hfc.getConfigSetting('peers'), hfc.getConfigSetting('channelName'), hfc.getConfigSetting('chaincodeName'), "placeBid", [args, "-"], req.username, req.orgname);
	res.send(message);
});

app.post('/add-asset', async function (req, res) {
	var args = req.body;
	if (!args) {
		res.send("error");
	}
	args = JSON.stringify(args);
	let message = await invoke.invokeChaincode(hfc.getConfigSetting('peers'), hfc.getConfigSetting('channelName'), hfc.getConfigSetting('chaincodeName'), "addAssetForBid", [args], req.username, req.orgname);
	res.send(message);
});
