...
func (t *AuctionChaincode) getBidResult(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	t.Errorf("[ getBidResult ] - Current Time %v", currentTime.String())
	if err != nil {
	...
	for resultsIterator.HasNext() {
		...
		t.Errorf("[ getBidResult ] - Current Asset Key for Bid Result %v", currentAssetKey)
		...
		t.Errorf("[ getBidResult ] - Current Asset Id for Bid Result %v", assetObj.AssetId)
...
func (t *AuctionChaincode) placeBid(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	//t.Errorf(fmt.Sprint("Found Asset String %v", foundAssetString))
	// asset is not sold
	if assetObj.IsSold {
	...
	if bidStartTime != nil {
		t.Errorf(fmt.Sprint("bidStartTime: %v", bidStartTime.String()))
	}

	if bidEndTime != nil {
		t.Errorf(fmt.Sprint("bidEndTime %v", bidEndTime.String()))
	}

	if bidTime != nil {
		t.Errorf(fmt.Sprint("bidTime: %v", bidTime.String()))
	}
	//TODO
...
func (t *AuctionChaincode) addAssetForBid(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	timeNow := time.Now()
	t.Errorf(fmt.Sprint("Current Time %v",timeNow.String()))
...
func (t *AuctionChaincode) declareWinnerForAsset(stub shim.ChaincodeStubInterface, assetObj *Asset) (error) {
	t.Errorf("[ declareWinnerForAsset ] - start for asset id %v", assetObj.AssetId)
	...
	t.Errorf("[ declareWinnerForAsset ] - hasBids %v", hasBids)
	...
	for availableBidsIterator.HasNext() {
		...
		t.Errorf("[ declareWinnerForAsset ] - currentBidKey %v", currentBidKey)

		_, currentBidKeyParts, _ := stub.SplitCompositeKey(currentBidKey)
		currentBidderEmail := currentBidKeyParts[1]

		t.Errorf("[ declareWinnerForAsset ] - currentBidderEmail %v", currentBidderEmail)
	...
	if hasBids {
		t.Errorf("[ declareWinnerForAsset ] - maxBidderEmail %v", maxBidderEmail)
		t.Errorf("[ declareWinnerForAsset ] - maxBid %v", maxBid.String())

		user, err := getUser(stub, maxBidderEmail)
		if err != nil {
			return err
		}
		//deduct the balance
		user.Balance.Sub(user.Balance, maxBid)
		t.Errorf("[ declareWinnerForAsset ] - userId %v has Balance %v ",user.UserId, user.Balance)
		...
		originalOwner, err := getUser(stub, originalOwnerEmail)
		if err != nil {
			return err
		}
		originalOwner.Balance.Add(originalOwner.Balance, maxBid)
		t.Errorf("[ declareWinnerForAsset ] - userId %v has Balance %v ",originalOwner.UserId, originalOwner.Balance)
